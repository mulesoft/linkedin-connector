...
public class LikesImpl
    ...
    public void init(XmlPullParser parser) throws IOException, XmlPullParserException {
        ...
        while (parser.nextTag() == XmlPullParser.START_TAG) {
            ...
            if (name.equals("like")) {
                ...
                likeImpl.init(parser);
                getLikeList().add(likeImpl);
            } else {
    ...
    public void toXml(XmlSerializer serializer) throws IOException {
        XmlSerializer element = serializer.startTag(null, "likes");
        XppUtils.setAttributeValueToNode(element, "total", getTotal());
        for (Like like : getLikeList()) {
            ((LikeImpl) like).toXml(serializer);
        }
        serializer.endTag(null, "likes");
    }
